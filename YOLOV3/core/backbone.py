...
def _stem_block(input_x):
    ...
    maxpool1_r0 = tf.keras.layers.Max2D(conv1_l1, pool_size=(2, 2), strides=(2,2), padding'=SAME')
    filter_concat = tf.concat([conv1_l1, maxpool1_r0], axis=-1)
    output = common.convolutional(filter_concat, (1, 1, 32, 32), downsample=True, activate=True, bn=True)
    return output
...
def _dense_block(input_x, stage,num_block, k, bottleneck_width):
    ...
    for index in range(num_block):
        ...
        # left channel
        conv_left_0 = common.convolutional(input_x, (1, 1,tf.cast(tf.shape(input_x)[-1], tf.int32) ,inter_channel), downsample=False, activate=True, bn=True)
        conv_left_1 = common.convolutional(conv_left_0, (3, 3, inter_channel,k), downsample=False, activate=True, bn=True)
        # right channel
        conv_right_0= common.convolutional(input_x, (1, 1,tf.cast(tf.shape(input_x)[-1], tf.int32) ,inter_channel), downsample=False, activate=True, bn=True)
        conv_right_1= common.convolutional(conv_right_0, (3, 3, inter_channel,k), downsample=False, activate=True, bn=True)
...
def _transition_layer(input_x, stage,output_channel, is_avgpool=True):
    ...
    conv0 = common.convolutional(input_x, (1, 1,tf.cast(tf.shape(input_x)[-1], tf.int32) ,output_channel), downsample=False, activate=True, bn=True)

    if is_avgpool:
       conv0 =  common.convolutional(input_x, (1, 1, output_channel, output_channel),downsample=True, activate=True, bn=True)
       output = conv0
